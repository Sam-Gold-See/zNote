...
`max()`,`min()`,`sum()`这 3 个内置函数用于计算列表、元组或者其他包含有限个元素的课迭代对象中所有元素的最大值、最小值以及所有元素之和
...
`sep.join(str_list)`：表示将列表中多个字符串进行连接（连接符为**sep 字符串**）
...
- 通过自动微分变量，Pytorch 可以将运算过程全部通过`.grad_fn`属性记录下来，构成一个计算图
