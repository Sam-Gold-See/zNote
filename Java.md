...
SpringCache 是一个框架，实现了基于**注解**的缓存功能，只需要简单添加一个注解实现缓存功能

Spring Cache 提供了一层抽象，底层可以切换不同的缓存实现，例如：Redis、EHCache、Caffeine 等

常用注解：

| 注解             | 说明                                                                                                                 |
| ---------------- | -------------------------------------------------------------------------------------------------------------------- |
| `@EnableCaching` | 开启 Spring Cache 功能，通常加在启动类上                                                                             |
...
Spring Task 是 Spring 框架提供的任务调度工具，可以按照约定时间自动执行某个代码逻辑

**作用**：定时自动执行某个代码逻辑

#### cron 表达式

cron 表达式其实是一个字符串，通过 cron 表达式可以**定义任务触发的时间**
...
在启动类处使用`@EnableScheduling`开启任务调度
...
在启动类上标注注解`@MapperScan("com.example.demo.mapper")`或者在每个 mapper 接口上标注注解`@Mapper`

每个 Mapper 接口继承`BaseMapper<Pojo>`，通过扫描实体类并基于反射获取实体类信息作为数据库表信息

> 类名驼峰转下划线作为表名
> 名为 id 的字段作为主键
> 变量名驼峰转下划线作为表的字段名

`@TableName("table_name")`注解标注在 Pojo 实体类上，来指定实体类对应的数据库表名

`@TabledId("id_name")`来指定表中的主键字段信息
可填入`type = IdType.{}`来设置，`AUTO`数据库自增长，`INPUT`

`@TableField("field_name")`来指定表中的普通字段信息（当成员变量名和数据库字段名不一致时使用；以及以 is 开头且为布尔值时使用；成员变量名与数据库关键字冲突时）
...
mybatis-plus:
  ...
  global-config:
    db-config:
      id-type: auto # 主键默认策略
      update-strategy: no_null # 字段更新策略：只更新非空字段
```
...
2. 在 mapper 方法参数中用 Param 注解声明 wrapper 变量名称，必须是 `ew`，有常量 `Constants.WRAPPER` 代替
...
微服务拆分之后必然涉及服务之间的相互调用，但是在大多数调用中，调用者发起请求后需要**等待**服务提供者执行业务返回结果，才能继续执行后续业务，所以调用者在调用过程中出于**阻塞状态**，因此这种调用方式为**同步调用**/**同步通讯**。但在实际开发过程中，我们可能需要采用**异步通讯**方式

异步调用方式就是基于消息通知的方式，一般包含三个角色：

- 消息发送者：投递消息的人，就是原来的调用方

- 消息 Broker：管理、暂存、转发信息，可以理解成微信服务器
...
![流控模式](img/Java_53.png)

链路策略是同一个请求项目限制，关联是不同请求相互限制
